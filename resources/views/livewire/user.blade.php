            ...
            <div>
                <x-jet-dialog-modal wire:model="userModal">
                    ...
                    <x-slot name="content">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control {{ $errors->has('role') ? 'is-invalid' : '' }}" wire:model="role" id="role" autocomplete="role">
                                @foreach ($roles as $r)
                                <option value="{!! $r->id !!}" wire:key="{{ $r->id }}" {!! $role == $r->id ? 'seledted' : '' !!}>{{ $r->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <x-slot name="footer">
                        <div class="d-flex">
                            ...
                            <x-jet-button class="ms-2" wire:click="{{ $aksiUserModal }}" wire:loading.attr="disabled">
                                <div wire:loading wire:target="{{ $aksiUserModal }}" class="spinner-border spinner-border-sm" role="status">
                ...
                <x-jet-confirmation-modal wire:model="deleteUserModalConfirm">
                    ...
                    <x-slot name="footer">
                        ...
                        <x-jet-danger-button wire:loading.attr="disabled" wire:click="deleteUser">
                            {{ __('Hapus') }}
                        </x-jet-danger-button>
                    </x-slot>
